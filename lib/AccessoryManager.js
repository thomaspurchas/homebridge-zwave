...
class AccessoryManager {
  ...
  /**
   * Configure the "Fan" serivce for the accessory
   * This is a custom hack
   */

   _configureFanService () {
      const { zwaveNodeId, displayName } = this._accessoryConfig

      const service = this._getService(this._hapService.Fanv2)
      const activeCharacteristic = service.getCharacteristic(this._hapCharacteristic.Active)
      const speedCharacteristic = service.getCharacteristic(this._hapCharacteristic.RotationSpeed)
      ...
      const boostValueCriteria = {
        commandClass: COMMAND_CLASS_SWITCH_BINARY,
        valueIndex: SWITCH_BINARY_INDEX_SWITCH,
        valueInstance: 1,
      }
      const boostValueResolver = this._makeZwaveNodeValueResolver(boostValueCriteria)
      const boostValueSetter = this._makeZwaveNodeValueSetter(boostValueCriteria)

      const setbackValueCriteria = {
        commandClass: COMMAND_CLASS_SWITCH_BINARY,
        valueIndex: SWITCH_BINARY_INDEX_SWITCH,
        valueInstance: 2,
      }
      const setbackValueResolver = this._makeZwaveNodeValueResolver(setbackValueCriteria)
      const setbackValueSetter = this._makeZwaveNodeValueSetter(setbackValueCriteria)

      speedCharacteristic
        .on('get', (callback) => {
          if (boostValueResolver()) {
            callback(null, 100)
          } else if (setbackValueResolver()) {
            callback(null, 0)
          } else {
            callback(null, 50)
          }
        ...
        .on('set', (value, callback) => {
          if (value == 100) {
            boostValueSetter(true)
            setbackValueSetter(false)
          } else if (value == 50) {
            boostValueSetter(false)
            setbackValueSetter(false)
          } else if (value == 0) {
            boostValueSetter(false)
            setbackValueSetter(true)
          }

          callback(null)
        })

      const boostValueId = this._zwave.generateNodeValueId({
        nodeId: zwaveNodeId,
        commandClass: COMMAND_CLASS_SWITCH_BINARY,
        valueIndex: SWITCH_BINARY_INDEX_SWITCH,
        valueInstance: 1,
      })
      this._zwave.onNodeValueChanged(boostValueId, value => {
        this._log(`${displayName} boost switch changed to ${value}`)
        if (value) {
          this._log(`${displayName} disabling setback and setting to 100%`)
          setbackValueSetter(false)
          speedCharacteristic.updateValue(100)
        } else if (setbackValueResolver()) {
          this._log(`${displayName} setting to 0%`)
          speedCharacteristic.updateValue(0)
        } else {
          this._log(`${displayName} setting to 50%`)
          speedCharacteristic.updateValue(50)
        }
      })
   }
  ...
  _makeZwaveNodeValueSetter ({ commandClass, valueInstance, valueIndex }) {
    return (newValue) => {
      const { zwaveNodeId, zwaveNodeInstance } = this._accessoryConfig
      ...
      const nodeValueId = this._zwave.generateNodeValueId({
        nodeId: zwaveNodeId,
        commandClass,
        valueInstance: valueInstance || zwaveNodeInstance,
        valueIndex
      })
